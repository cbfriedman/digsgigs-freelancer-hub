...
serve(async (req) => {
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabaseClient
      ...
      .select('id, profession, keywords, industry_type, user_id')
    ...
    // Calculate call price (100% of Google high PPC)
    const keyword = profile.profession || profile.industry_type || 'general contractor';
    const cpcData = findCpcData(keyword);
    const costDollars = roundToHalfDollar(cpcData.highCpc * PROFILE_CALL_MULTIPLIER);
    const costCents = Math.round(costDollars * 100);

    logStep("Price calculated", { costCents, highCpc: cpcData.highCpc });
    ...
    const { data: callRecord, error: insertError } = await supabaseClient
      ...
      .insert({
        ...
        google_high_cpc_cents: Math.round(cpcData.highCpc * 100),
      })
      .select()
      .single();
    ...
    await supabaseClient.rpc('create_notification', {
      ...
      p_title: 'New Profile Call',
      p_message: `Someone called your profile. Call charge: $${costDollars.toFixed(2)}`,
      p_type: 'profile_call',
