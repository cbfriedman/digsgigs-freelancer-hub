-- Fix: Cleanup failed ... violates foreign key constraint "escrow_contracts_consumer_id_fkey"
-- escrow_contracts.consumer_id references public.profiles(id), so we must delete those rows
-- before deleting the profile. milestone_payments CASCADE from escrow_contracts; transactions/disputes SET NULL.
...
BEGIN
  ...
  -- 4) Tables that reference profiles(id): delete escrow_contracts for this consumer first
  --    (milestone_payments CASCADE; transactions/disputes escrow_contract_id SET NULL)
  DELETE FROM public.escrow_contracts WHERE consumer_id = target_user_id;

  -- 5) Remove the profile row so auth.admin.deleteUser is not blocked by public.profiles
  DELETE FROM public.profiles WHERE id = target_user_id;
END $$;

COMMENT ON FUNCTION public.admin_clear_user_references(uuid) IS
  'Clears all refs to the user (auth, storage, public, escrow, profiles) so auth.admin.deleteUser can succeed.';
