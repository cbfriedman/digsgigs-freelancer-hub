-- Clear auth.mfa_factors so users with 2FA can be deleted (GoTrue blocks auth.users delete otherwise).
...
DECLARE
  ...
  total_affected int;
BEGIN
  -- 0) Auth schema: remove sessions, identities, refresh_tokens, and mfa_factors
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN NULL;
  END;
  -- MFA: delete challenges for this user's factors first, then factors (both can block auth.users delete)
  BEGIN
  ...
  -- 3) Delete/clear rows that reference the user (so auth.admin.deleteUser is not blocked)
...
COMMENT ON FUNCTION public.admin_clear_user_references(uuid) IS
  'Clears auth sessions/identities/refresh_tokens/mfa_factors, storage, and public refs so auth.admin.deleteUser can succeed.';

-- Include auth.mfa_factors in diagnostic.
