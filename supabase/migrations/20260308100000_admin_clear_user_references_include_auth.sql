-- Clear auth.sessions and auth.identities before other cleanup so auth.admin.deleteUser is not blocked.
...
DECLARE
  ...
  total_affected int;
BEGIN
  -- 0) Auth schema: remove sessions and identities so delete from auth.users can succeed
  BEGIN
...
COMMENT ON FUNCTION public.admin_clear_user_references(uuid) IS
  'Clears auth sessions/identities, storage, and public refs so auth.admin.deleteUser can succeed.';
