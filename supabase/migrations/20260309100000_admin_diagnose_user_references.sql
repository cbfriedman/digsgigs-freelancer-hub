-- Returns rows that still reference the user (for debugging delete failures).
-- Call when auth.admin.deleteUser fails to see which table is blocking.
...
COMMENT ON FUNCTION public.admin_list_remaining_user_references(uuid) IS
  'Returns tables that still have rows referencing the user (for debugging delete failures).';
