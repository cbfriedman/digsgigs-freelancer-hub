-- Return consumer full name in get_my_conversations so freelancer (digger) sees client name in chat list.
...
COMMENT ON FUNCTION public.get_my_conversations() IS
  'Returns all conversations with last message, partner avatar URLs, consumer name, unread count, muted, and is_blocked.';
