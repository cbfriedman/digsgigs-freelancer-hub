  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const id of toDelete.map((e) => e.id)) {
        await (supabase.from("digger_experience" as any)).delete().eq("id", id);
      }
      ...
      for (let i = 0; i < valid.length; i++) {
        ...
        if (item.id && !item.id.startsWith("draft-") && existingIds.has(item.id)) {
          await (supabase.from("digger_experience" as any)).update(payload).eq("id", item.id);
        } else {
          await (supabase.from("digger_experience" as any)).insert({
          ...
          {isEditing && (
            <Card className="border-primary/30">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="exp-employment-type">Employment type</Label>
                    <select
                      id="exp-employment-type"
                      value={draft.employment_type || ""}
                      onChange={(e) => updateDraft("employment_type", e.target.value)}
