-- =====================================================
-- COMPLETE TAXONOMY SETUP SCRIPT
...
DELETE FROM profession_requests;
...
-- Check industry categories count (should be 16)
-- SELECT COUNT(*) as category_count FROM industry_categories;

-- Check professions count (should be ~95)
-- SELECT COUNT(*) as profession_count FROM professions;

-- Check professions by category
-- SELECT ic.name, COUNT(p.id) as profession_count 
-- FROM industry_categories ic
-- LEFT JOIN professions p ON p.industry_category_id = ic.id
-- GROUP BY ic.name
